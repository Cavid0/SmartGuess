(function () {
    ...
    const _SC = {
        _k: 0, _v: [],
        set(arr) { this._k = (Math.random() * 0xffff) | 0 || 1; this._v = arr.map(d => d ^ this._k); },
        get()    { return this._v.map(x => x ^ this._k); },
        at(i)    { return this._v[i] ^ this._k; }
    };
    ...
    function setupEventListeners() {
        ...
        elements.inputs.forEach((input, index) => {
            input.addEventListener('input', (e) => {
                ...
                if (e.target.value && index < 3) elements.inputs[index + 1].focus();
            });

            input.addEventListener('keydown', (e) => {
                if (e.key === 'Backspace' && !e.target.value && index > 0) elements.inputs[index - 1].focus();
    ...
    function handlePaste(e) {
        ...
        if (numbers.length > 0 && numbers.length < 4) elements.inputs[numbers.length].focus();
        else if (numbers.length === 4) elements.submitBtn.focus();
    }

    function initializeGame() {
        _SC.set(Array.from({ length: 4 }, () => Math.floor(Math.random() * 10)));
        attempts = 0;
        gameActive = true;
    ...
    function logAttempt(guess, feedback) {
        ...
        if (feedback.every(f => f === 'correct')) item.classList.add('win');
    ...
    function endGame(isWin) {
        ...
        if (isWin) { window.Sfx && Sfx.win(); window.Hapt && Hapt.success(); }
        else       { window.Sfx && Sfx.lose(); window.Hapt && Hapt.error(); }
        ...
        setTimeout(() => {
            ...
            elements.secretCodeDisplay.textContent = _SC.get().join('');
